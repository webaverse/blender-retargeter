...
class RetargetingPanel(ToolPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row(align=True)
        row.prop(context.scene, 'rsl_retargeting_armature_source')

        row = layout.row(align=True)
        row.prop(context.scene, 'rsl_retargeting_armature_target')
        ...
        if not anim_exists:
            row = layout.row(align=True)
            row.label(text='No animated armature found!')
            return
        ...
        if not context.scene.rsl_retargeting_bone_list:
            row = layout.row(align=True)
            row.scale_y = 1.2
            row.operator(retargeting.BuildBoneList.bl_idname)
            self.draw_import_export(layout)
            return
        ...
        row.scale_y = 1.2
        row.operator(retargeting.BuildBoneList.bl_idname, text='Rebuild Bone List')
        row = subrow.row(align=True)
        row.scale_y = 1.2
        row.alignment = 'RIGHT'
        row.operator(retargeting.ClearBoneList.bl_idname, text="")
        ...
        row = layout.row(align=True)
        row.operator(retargeting.AddBoneListItem.bl_idname, text="Add Custom Entry")
        ...
        row = layout.row(align=True)
        row.scale_y = 1.4
        row.operator(retargeting.RetargetAnimation.bl_idname)
    ...
    def draw_import_export(self, layout):
        ...
        row.scale_y = 0.9
        row.alignment = 'RIGHT'
        row.operator(detector.ClearCustomBones.bl_idname, text='')
